...
from utils import DiffLoss, CMD, SimilarityKL
...
checkpoint_path = "/home/jack/Project/MutiModal/SentimentAnalysis/JackNet/checkpoints/AOTN/"
log_file = "/home/jack/Project/MutiModal/SentimentAnalysis/JackNet/checkpoints/AOTN/MutimodalSALog.txt"
...
class Solver(object):
    ...
    def train(self):
        ...
        for e in range(start_epoch,self.train_config.n_epoch): # 0-100
            ...
            with open(log_file, "a") as file:
                ...
                file.write(f"Epoch {e} - Training loss: {round(np.mean(train_loss), 4)}\n")
    ...
    def calc_metrics(self, y_true, y_pred, mode=None, to_print=False): # Jack Change 反了  害群之马  0531 你change之后会让none-zero变为ture里面的，完全混乱！
        """
        ...
        if mode=="train": # Jack Add 0531 pos/neg 也要提上日程
            pass
        if mode=="dev":
            print("val_Accuracy (pos/neg) ", accuracy_score(binary_truth, binary_preds))
            with open(log_file, "a") as file:
                    file.write(f"val_Accuracy (pos/neg) {accuracy_score(binary_truth, binary_preds)}\n")        
        if mode=="test":
