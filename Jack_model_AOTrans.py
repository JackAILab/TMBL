...
class PS_Mixer(nn.Module):
    def __init__(self, config):
        ...
        # Transformer
        ##########################################
        self.tf_encoder_av = MyTransformer(d_emb_1=4*input_sizes[2], d_emb_2=4*input_sizes[1], n_layers=5, d_inner=512, n_head=8, d_k=32, d_out=64, dropout=0.5, n_position=30, add_sa=True) # None -- 512/1024 Jack Change
        self.tf_encoder_at = MyTransformer(d_emb_1=4*input_sizes[2], d_emb_2=768, n_layers=5, d_inner=512, n_head=8, d_k=32, d_out=64, dropout=0.5, n_position=30, add_sa=True)
        self.tf_encoder_vt = MyTransformer(d_emb_1=4*input_sizes[1], d_emb_2=768, n_layers=5, d_inner=512, n_head=8, d_k=32, d_out=64, dropout=0.5, n_position=30, add_sa=True)
        # 对齐之后，三模态采用一个共享 Transformer, 以文本模态为中心进行设计 
        self.tf_encoder_share = MyTransformer(d_emb_1=self.config.hidden_size, d_emb_2=self.config.hidden_size, n_layers=5, d_inner=512, n_head=8, d_k=32, d_out=64, dropout=0.5, n_position=30, add_sa=True)
        # FC
    ...
    def alignment(self, visual, acoustic, lengths, bert_sent, bert_sent_type, bert_sent_mask): # MOSEI: visual([59, 32, 35]) & acoustic([59, 32, 74]) & lengths([32]) & ([32, 61])
        ...
        # ==== Jack Add 使用 AOTrans 获得的对齐多模态 对 share_modality 进行压缩 Version 2 采用 三个共同 share 的策略
        h1 = self.share_T
        h2 = self.share_V
        h1 = torch.cat((h1[0], h1[1]), dim=1) # ([32, 512])
        h2 = torch.cat((h2[0], h2[1]), dim=1)
    ...
    def shared_modaties(self, utterance_t, utterance_v, utterance_a):
        ...
        # self.share_T2, self.share_A2 = self.tf_encoder_share(torch.stack([utterance_t,utterance_v],dim=0), torch.stack([utterance_a,utterance_v],dim=0)) # ([2, 32, 128]) -> ([2, 32, 128])
