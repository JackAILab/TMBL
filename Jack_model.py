...
class PS_Mixer(nn.Module):
    def __init__(self, config):
        ...
        # encoder_layer2 = nn.TransformerEncoder Layer(d_model=self.config.hidden_size, nhead=2)
    ...
    def alignment(self, visual, acoustic, lengths, bert_sent, bert_sent_type, bert_sent_mask): # MOSEI: visual([59, 32, 35]) & acoustic([59, 32, 74]) & lengths([32]) & ([32, 61])
        ...
        self.shared_modaties(utterance_text, utterance_video, utterance_audio) # 模态对齐->模态共享 ([32, 128]) 
        ...
        h1 = h1 * (torch.div(norm2.unsqueeze(1), norm1.unsqueeze(1))) # h1*投影比例([32, 1])
    ...
    def shared_modaties(self, utterance_t, utterance_v, utterance_a):
        ...
        self.utt_t_orig = utterance_t = self.project_t(utterance_t) # ([32, 768]) -> ([32, 128])
        self.utt_v_orig = utterance_v = self.project_v(utterance_v) # ([32, 140]) -> ([32, 128])
        self.utt_a_orig = utterance_a = self.project_a(utterance_a) # ([32, 296]) -> ([32, 128]) 
