...
class MOSEI:
    def __init__(self, config):
        ...
        try:
            ...
            # define your different modalities - refer to the filenames of the CSD files
            visual_field = 'CMU_MOSEI_VisualOpenFace2' # 42->2
...
class UR_FUNNY:
    def __init__(self, config):
        ...
        try:
            ...
            self.dev = load_pickle(DATA_PATH + '/dev.pkl')
            self.test = load_pickle(DATA_PATH + '/test.pkl')
            self.pretrained_emb, self.word2id = torch.load(CACHE_PATH) # UR_FUNNY V1 版本数据 无需进行 glove embedding
            ...
            word_embedding_idx_sdk=load_pickle(DATA_PATH + "/word_embedding_indexes_sdk.pkl") # 不包含文本数据，直接被转换为矩阵了
            # word_to_idx_sdk =load_pickle(DATA_PATH + "/word_to_idx.pkl") # Jack Add 0523 使用修复V2版本的UR_FUNNY
            word_list_sdk=load_pickle(DATA_PATH + "/word_embedding_list.pkl")
            humor_label_sdk = load_pickle(DATA_PATH + "/humor_label_sdk.pkl")
            # language_sdk = load_pickle(DATA_PATH + "/language_sdk.pkl") # Jack Add 0523 修复V2版本加入了每个话语对应的文本
            ...
            self.test = test = []
            self.word2id = word2id
            # self.pretrained_emb, self.word2id = torch.load(CACHE_PATH) # Jack Add
            ...
            for key in humor_label_sdk.keys():
                ...
                label = np.array(humor_label_sdk[key], dtype=int) # 1
                _word_id = np.array(word_embedding_idx_sdk[key]['punchline_embedding_indexes']) # array([32288, 21046])
                # _word_id_text = np.array(language_sdk[key+1]['punchline_sentence']) # Jack Add 使用话语文本
                _acoustic = np.array(word_aligned_covarep_sdk[key]['punchline_features']) # (2, 81)
                _visual = np.array(word_aligned_openface_sdk[key]['punchline_features']) # (2, 75)
                ...
                for i, word_id in enumerate(_word_id):
                    ...
                    #     acoustic.append(_acoustic[i, :])
                    # Original
                    word = word_list_sdk[word_id] # (300,)
                    actual_words.append(word)
                    # actual_words.append(_word_id_text[0].decode('utf-8')) # Jack Change
                    # words.append(word2id[word]) # TypeError: unhashable type: 'numpy.ndarray'
                    words.append(word) # Jack Change
                    visual.append(_visual[i, :])
                    acoustic.append(_acoustic[i, :])

                words = np.asarray(words) # (2, 300), ..., (6, 300)
                visual = np.asarray(visual) # (2, 75) 
                acoustic = np.asarray(acoustic) # (2, 81)
